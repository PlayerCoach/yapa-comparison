...
def create_set(data_path, accent_regex, name, size, inject_polish=False):
    tsv_path = os.path.join(data_path, TSV_FILE)
    clips_path = os.path.join(data_path, CLIPS_DIR)
    output_dir = os.path.join("data", f"{name}_clips")
    output_tsv = os.path.join("data", f"{name}.tsv")
...
if __name__ == "__main__":
    ...
    create_set(DATA_PATH, r"^England English$", "british", args.size)
    create_set(DATA_PATH, r"^United States English$", "american", args.size)
    create_set(
        DATA_PATH,
